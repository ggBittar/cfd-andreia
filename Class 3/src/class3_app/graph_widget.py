...
from .burgers_backend import evaluate_exact_curve, solve_burgers_space_time
from .function_catalog import NamedFunction
from .space_time_method_catalog import SpaceTimeMethod
...
def method_style(method_id: str) -> tuple[Qt.PenStyle, float]:
    styles = {
        "ftbs": (Qt.PenStyle.DashDotLine, 2.0),
        "ftcs": (Qt.PenStyle.DotLine, 2.0),
        "lax_friedrichs": (Qt.PenStyle.DashLine, 2.4),
    }
...
class GraphWidget(QWidget):
    def __init__(self, parent: QWidget | None = None) -> None:
        ...
        self.tf = 0.0
        self.n_value = 1
        self.nx_values: list[int] = []
        self.nt_values: list[int] = []
        ...
        self.selected_methods: list[SpaceTimeMethod] = []
    ...
    def set_plot_definition(
        ...
        nx_values: list[int],
        nt_values: list[int],
        selected_methods: list[SpaceTimeMethod],
    ) -> None:
        ...
        self.nx_values = list(nx_values)
        self.nt_values = list(nt_values)
    ...
    def recompute_curves(self) -> None:
        ...
        exact_curve = evaluate_exact_curve(self.selected_function.evaluator, x_values, self.tf, self.n_value)
        self._append_curve(
            label=f"Exata tf | N = {self.n_value}",
            x_values=x_values,
            y_values=exact_curve,
        ...
        combination_index = 0
        for nx_value in self.nx_values:
            for nt_value in self.nt_values:
                for method in self.selected_methods:
                    result = solve_burgers_space_time(
                    ...
                    self._append_curve(
                        label=f"{method.label} | Nx = {nx_value}, Nt = {nt_value}",
                        x_values=result.x_values,
                        y_values=result.y_values,
                        color=color_for_index(combination_index),
                        pen_style=pen_style,
                        width=width,
                    )
                    combination_index += 1
    ...
    def paintEvent(self, event: QPaintEvent) -> None:  # noqa: N802
        ...
        legend_box = QRectF(plot_rect.right() - 320, plot_rect.top() + 10, 310, legend_height)
        painter.setPen(QColor(58, 66, 84))
        painter.setBrush(QColor(16, 20, 26, 215))
        painter.drawRoundedRect(legend_box, 8.0, 8.0)
        legend_y = int(legend_box.top()) + 14
        for curve in self.curves:
            if legend_y > legend_box.bottom() - 12:
                painter.setPen(QColor(214, 220, 230))
                painter.drawText(
                    QRectF(legend_box.left() + 12, legend_box.bottom() - 20, legend_box.width() - 24, 16),
        ...
        painter.drawText(
            ...
            f"Class 3 | tf = {self.tf:.3f}, N = {self.n_value}, comparacao espaco-temporal",
        )
